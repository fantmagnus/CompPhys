...
void time_scaling(double T_eq, double time, double timestep,  double *v[][3], double tau_TO);

void time_scaling(double T_eq, double T, double timestep,  double *v[][3], double tau_T){
...
{ 
  ...
  for (int i = 1; i < nbr_of_timesteps + 1; i++) {
    ...
    /* Calculate instantaneous temperature using equipartition theorem */ 
    Temp[i] = 2 / (3*nbr_of_atoms*k_B) * E_kin[i];
    /* Temperature scaling */
    for(int j = 0; j < nbr_of_atoms; j++) {
    time_scaling(T_eq, i*timestep, timestep, v[j][], tau_T);
    }

  }
  
  /* Print temperature and energy data to output file */
